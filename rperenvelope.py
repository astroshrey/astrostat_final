...
'Prot': np.log10(Prot), 'Rper': np.log10(Rper), 'Fe/H': FeH, 'rad': rad, 'rho': rho, 'L': L})
...
window = 2000
down, smooth, up = movingmedian(df['Rper'], window)
...
plt.scatter(df['Prot'], df['Rper'], c = 'k', s = 0.2)
#plt.xscale('log')
#plt.yscale('log')
plt.xlabel(r'P$_{rot}$ [days]')
plt.ylabel(r'R$_{per}$ [ppm]$')
#plt.ylim(10**6, 10**12)
plt.xlim(min(x), max(x))
plt.savefig(output_dir+"envelope_4.png", format='png')
